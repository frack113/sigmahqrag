...
class TestComponentIntegration:
    ...
    @pytest.mark.asyncio
    async def test_chat_data_integration(
        self, chat_interface, data_management, sample_text_file
    ):
    ...
    @pytest.mark.asyncio
    async def test_chat_history_integration(
        self, chat_interface, mock_chat_history_service
    ):
...
class TestServiceIntegration:
    ...
    async def test_llm_rag_integration(self, mock_llm_service, mock_rag_service):
        ...
        mock_rag_service.query = AsyncMock(
            return_value=[{"text": "Relevant content", "score": 0.9, "metadata": {}}]
        )
    ...
    @pytest.mark.asyncio
    async def test_data_file_processor_integration(
        self, mock_data_service, sample_text_file
    ):
    ...
    def test_config_logging_integration(
        self, mock_config_service, mock_logging_service
    ):
...
class TestPerformanceIntegration:
    ...
    async def test_concurrent_chat_requests(self, mock_chat_service):
        ...
        assert len(results) == 5
        assert all(
            f"Response to: Message {i}" in result for i, result in enumerate(results)
        )
