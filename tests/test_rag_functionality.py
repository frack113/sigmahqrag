...
class TestRAGDocumentProcessing:
    ...
    def test_text_chunking_strategy(self, file_processor):
        ...
        chunks_100 = file_processor.chunk_text(
            long_text, chunk_size=100, chunk_overlap=20
        )
        chunks_200 = file_processor.chunk_text(
            long_text, chunk_size=200, chunk_overlap=40
        )
    ...
    async def test_document_processing_pipeline(self, data_service, temp_dir):
        ...
        for i in range(3):
            ...
            content = (
                f"Document {i} content. This is test content for document {i}.\n" * 20
            )
...
class TestRAGQueryFunctionality:
    ...
    @pytest.mark.asyncio
    async def test_rag_query_with_relevant_results(
        self, mock_rag_service, mock_llm_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_rag_query_with_score_filtering(
        self, mock_rag_service, mock_llm_service
    ):
...
class TestRAGChatIntegration:
    ...
    def chat_service_with_rag(self, mock_llm_service, mock_rag_service):
        ...
        mock_llm_service.generate_completion = AsyncMock(
            return_value="LLM response with context"
        )
    ...
    async def test_chat_with_rag_context(
        self, chat_service_with_rag, mock_llm_service, mock_rag_service
    ):
        ...
        mock_rag_service.query.assert_called_once_with(
            "Current question", n_results=3, min_score=0.1
        )
...
class TestRAGErrorHandling:
    ...
    async def test_rag_query_failure_handling(self, mock_rag_service):
        ...
        # Mock query failure
        mock_rag_service.is_initialized = True
        mock_rag_service.collection.query = AsyncMock(
