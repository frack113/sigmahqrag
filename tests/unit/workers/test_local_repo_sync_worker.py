...
class TestLocalRepoSyncWorker:
    def test_syncs_new_repos(self, mock_db: MagicMock) -> None:
        ...
        with patch("src.workers.sigma.sync_worker.list_repos") as mock_list:
            mock_list.return_value = [
                {"org": "test-org", "name": "repo-a", "remote_url": "https://example.com/a.git"},
            ]
            with patch("src.workers.sigma.sync_worker.get_metadata", return_value=None):
                with patch("src.workers.sigma.sync_worker.save_metadata") as mock_save:
    ...
    def test_skips_existing_repos(self, mock_db: MagicMock) -> None:
        ...
        with (
            patch("src.workers.sigma.sync_worker.list_repos") as mock_list,
            patch(
                "src.workers.sigma.sync_worker.get_metadata",
                return_value={"remote_head": "abc123"},
            ),
            patch("src.workers.sigma.sync_worker.save_metadata") as mock_save,
        ):
            mock_list.return_value = [
    ...
    def test_handles_dispatcher_state(self, mock_db: MagicMock) -> None:
        ...
        with patch("src.workers.sigma.sync_worker.list_repos", return_value=[]):
