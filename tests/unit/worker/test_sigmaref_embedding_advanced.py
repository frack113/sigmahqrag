"""Advanced tests for SigmaRefEmbeddingWorker — remaining coverage lines."""
...
class TestSigmaRefEmbeddingWorkerAdvanced:
    def test_get_entries_skips_missing_url_hash(self, mock_db: MagicMock) -> None:
        ...
        entries = worker._get_entries({"task_id": "t1"})
    ...
    def test_resolve_file_path_skips_when_candidate_equals_registry(
        ...
        result = worker._resolve_file_path({"file_name": "test-name.md"})
    ...
    def test_resolve_file_path_skips_registry_path_candidate(
        ...
        result = worker._resolve_file_path({"hash": "somehash", "file_name": ""})
        assert result is not None
