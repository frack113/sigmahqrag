...
Migrates the NiceGUI Data page to Gradio with async background operations
...
class DataManagement(AsyncComponent):
    ...
    def create_tab(self):
        ...
        with gr.Column(elem_classes="data-container"):
            ...
            with gr.Row():
                ...
                # Right column: Repository status
                with gr.Column(scale=1):
                    self.repo_display = gr.JSON(
                        value={}, 
                        label="Repository Status",
                        elem_classes="repo-json"
                    )
            ...
            self.refresh_btn.click(
                fn=lambda: self._auto_refresh_data(),
                inputs=[],
                outputs=[self.progress_text, self.stats_display, self.repo_display],
                queue=True
            )
    ...
    async def _auto_refresh_data(self):
        ...
        try:
            ...
            return "Data loaded successfully!", stats, repo_config
    ...
    def _setup_event_handlers(self):
        ...
        self.update_btn.click(
            fn=self._update_database_with_progress,
            inputs=[],
            outputs=[self.progress_text, self.stats_display, self.repo_display],
            queue=True
        )
    ...
    async def _update_database_with_progress(self) -> AsyncGenerator[tuple, None]:
        ...
        try:
            ...
            if not repo_config["repositories"]:
                yield "No repositories configured", self.stats_cache, repo_config
                return
            
            enabled_repos = [repo for repo in repo_config["repositories"] if repo["enabled"]]
            
            if not enabled_repos:
                yield "No enabled repositories found", self.stats_cache, repo_config
                return
            
            # Step 2: Clone repositories (Step 1/2)
            yield f"Step 1/2: Updating {len(enabled_repos)} repositories...", self.stats_cache, repo_config
            ...
            if not clone_result:
                yield "Failed to update repositories", self.stats_cache, repo_config
                return
            
            # Step 3: Index repositories (Step 2/2)
            yield "Step 2/2: Indexing repository content...", self.stats_cache, repo_config
            ...
            if index_result:
                ...
                yield success_msg, updated_stats, repo_config
            else:
                yield "No repositories were indexed", self.stats_cache, repo_config
                
        except Exception as e:
            error_msg = f"Error updating knowledge base: {str(e)}"
            logger.error(error_msg)
            yield error_msg, self.stats_cache, repo_config
    ...
    async def _refresh_data(self) -> AsyncGenerator[tuple, None]:
        ...
        try:
            ...
            yield "Data refreshed successfully!", stats, repo_config
