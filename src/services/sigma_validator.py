...
class SigmaValidator:
    ...
    def _validate_field_types(self, data: dict[str, Any]) -> None:
        ...
        if not isinstance(data.get("description"), str) or not data["description"].strip():
            raise ValidationError(field="description", message="Description must be a non-empty string")
    ...
    def _validate_condition_syntax(self, data: dict[str, Any]) -> None:
        ...
        if condition and isinstance(condition, str):
            ...
            invalid_refs = condition_words - detection_keys - {"and", "or", "not", "1", "of", "them"}
            if invalid_refs and not condition.startswith("selection"):
                logger.warning(f"Condition may reference non-existent detection keys: {invalid_refs}")
