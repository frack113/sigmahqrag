...
class EnvironmentSetup:
        ...
        try:
            ...
            config = {
                ...
                "database": {
                    ...
                    "backup_enabled": self.default_env_vars[
                        "DATABASE_BACKUP_ENABLED"
                    ].lower()
                    == "true",
                    "backup_interval_hours": int(
                        self.default_env_vars["DATABASE_BACKUP_INTERVAL"]
                    ),
                },
                "lm_studio": {
                    ...
                    "temperature": float(
                        self.default_env_vars["LM_STUDIO_TEMPERATURE"]
                    ),
                },
                "gradio": {
                ...
                "rag": {
                    ...
                    "conversation_history_limit": int(
                        self.default_env_vars["RAG_CONVERSATION_HISTORY_LIMIT"]
                    ),
                },
                "file_processing": {
                    "upload_max_size_mb": int(self.default_env_vars["UPLOAD_MAX_SIZE"]),
                    "allowed_types": self.default_env_vars[
                        "UPLOAD_ALLOWED_TYPES"
                    ].split(","),
                    "max_concurrent": int(
                        self.default_env_vars["PROCESSING_MAX_CONCURRENT"]
                    ),
                ...
                "performance": {
                    "max_memory_usage_mb": int(
                        self.default_env_vars["MAX_MEMORY_USAGE"]
                    ),
                    "cache_enabled": self.default_env_vars["CACHE_ENABLED"].lower()
                    == "true",
                    "cache_ttl": int(self.default_env_vars["CACHE_TTL"]),
                    "streaming_enabled": self.default_env_vars[
                        "STREAMING_ENABLED"
                    ].lower()
                    == "true",
                },
    ...
    def _get_default_config(self) -> dict[str, Any]:
        ...
        return {
            ...
            "performance": {
                ...
                == "true",
                "cache_ttl": int(self.default_env_vars["CACHE_TTL"]),
                "streaming_enabled": self.default_env_vars["STREAMING_ENABLED"].lower()
                == "true",
            },
        }
    ...
    def validate_configuration(self, config: dict[str, Any]) -> dict[str, bool]:
        ...
        validation_results["app_name"] = (
            isinstance(app_config.get("name"), str)
            and len(app_config.get("name", "")) > 0
        )
        validation_results["app_version"] = (
            isinstance(app_config.get("version"), str)
            and len(app_config.get("version", "")) > 0
        )
        validation_results["debug"] = isinstance(app_config.get("debug"), bool)
        validation_results["log_level"] = app_config.get("log_level") in [
            "DEBUG",
            "INFO",
            "WARNING",
            "ERROR",
        ...
        validation_results["database_path"] = (
            isinstance(db_config.get("path"), str)
            and len(db_config.get("path", "")) > 0
        )
        validation_results["backup_enabled"] = isinstance(
            db_config.get("backup_enabled"), bool
        )
        validation_results["backup_interval"] = (
            isinstance(db_config.get("backup_interval_hours"), int)
            and db_config.get("backup_interval_hours") > 0
        )

        # Validate LM Studio settings
        lm_config = config.get("lm_studio", {})
        validation_results["lm_studio_url"] = (
            isinstance(lm_config.get("url"), str) and len(lm_config.get("url", "")) > 0
        )
        validation_results["lm_studio_timeout"] = (
            isinstance(lm_config.get("timeout"), int) and lm_config.get("timeout") > 0
        )
        validation_results["lm_studio_max_tokens"] = (
            isinstance(lm_config.get("max_tokens"), int)
            and lm_config.get("max_tokens") > 0
        )
        validation_results["lm_studio_temperature"] = (
            isinstance(lm_config.get("temperature"), (int, float))
            and 0 <= lm_config.get("temperature") <= 2
        )

        # Validate Gradio settings
        gradio_config = config.get("gradio", {})
        validation_results["gradio_host"] = (
            isinstance(gradio_config.get("host"), str)
            and len(gradio_config.get("host", "")) > 0
        )
        validation_results["gradio_port"] = (
            isinstance(gradio_config.get("port"), int)
            and 1024 <= gradio_config.get("port") <= 65535
        )
        validation_results["gradio_share"] = isinstance(
            gradio_config.get("share"), bool
        )
        validation_results["gradio_debug"] = isinstance(
            gradio_config.get("debug"), bool
        )
        validation_results["gradio_theme"] = (
            isinstance(gradio_config.get("theme"), str)
            and len(gradio_config.get("theme", "")) > 0
        )

        # Validate RAG settings
        rag_config = config.get("rag", {})
        validation_results["rag_enabled"] = isinstance(rag_config.get("enabled"), bool)
        validation_results["rag_n_results"] = (
            isinstance(rag_config.get("n_results"), int)
            and rag_config.get("n_results") > 0
        )
        validation_results["rag_min_score"] = (
            isinstance(rag_config.get("min_score"), (int, float))
            and 0 <= rag_config.get("min_score") <= 1
        )
        validation_results["rag_chunk_size"] = (
            isinstance(rag_config.get("chunk_size"), int)
            and rag_config.get("chunk_size") > 0
        )
        validation_results["rag_chunk_overlap"] = (
            isinstance(rag_config.get("chunk_overlap"), int)
            and rag_config.get("chunk_overlap") >= 0
        )
        validation_results["rag_conversation_limit"] = (
            isinstance(rag_config.get("conversation_history_limit"), int)
            and rag_config.get("conversation_history_limit") > 0
        )
    ...
    def create_environment_summary(self) -> str:
        ...
        summary.append(
            f"Environment Variables Set: {info['environment_variables_set']}"
        )
        summary.append("")
