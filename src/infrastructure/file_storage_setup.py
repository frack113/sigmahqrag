...
class FileStorageSetup:
    ...
    def create_sample_files(self) -> dict[str, bool]:
        ...
        sample_configs = {
            "config": [
                ...
                ("database.json", '{"path": "data/local/sigmahq.db", "backup_enabled": true}'),
                ("logging.json", '{"level": "INFO", "file": "logs/app.log"}')
            ],
            "logs": [
                ("readme.txt", "Application logs are stored in this directory.")
            ],
            "cache": [
                ("readme.txt", "Temporary cache files are stored in this directory.")
            ],
            "backups": [
                ("readme.txt", "Database and configuration backups are stored in this directory.")
            ]
        }
            ...
            if dir_name in self.directories:
                ...
                for filename, content in files:
                    try:
                        ...
                        self.logger.error(f"Failed to create sample file {filename} in {dir_name}: {e}")
    ...
    def validate_storage_setup(self) -> dict[str, Any]:
        ...
        validation_results["validation_passed"] = (
            validation_results["directories_created"] and
            validation_results["permissions_set"]
        )
