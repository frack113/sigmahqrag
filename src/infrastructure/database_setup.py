    ...
    def create_database_schema(self) -> bool:
        ...
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute("CREATE INDEX IF NOT EXISTS idx_config_key ON configuration(key)")
                cursor.execute("CREATE INDEX IF NOT EXISTS idx_logs_timestamp ON application_logs(timestamp)")
                cursor.execute("CREATE INDEX IF NOT EXISTS idx_logs_level ON application_logs(level)")
                cursor.execute("CREATE INDEX IF NOT EXISTS idx_github_status ON github_repositories(status)")
                cursor.execute("CREATE INDEX IF NOT EXISTS idx_file_ops_type ON file_operations(operation_type)")
                cursor.execute("CREATE INDEX IF NOT EXISTS idx_file_ops_status ON file_operations(status)")
    ...
    def test_database_connection(self) -> bool:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute("SELECT value FROM configuration WHERE key = ?", (test_key,))
