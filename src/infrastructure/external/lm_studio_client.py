...
from ...shared.constants import SERVICE_LM_STUDIO
...
class LMStudioClient(BaseService):
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if attempt == self.max_retries - 1:  # Last attempt
                    self.logger.error(f"LM Studio API request failed after {self.max_retries} attempts: {e}")
    ...
    async def generate_streaming_completion(
        ...
        try:
            ...
            response = requests.post(url, headers=headers, json=data, timeout=self.timeout, stream=True)
            response.raise_for_status()
