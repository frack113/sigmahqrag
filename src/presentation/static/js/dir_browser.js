...
class DirBrowser {
    ...
    _delegateEvents() {
        this._boundChange = function (e) {
            var cb = e.target;
            if (cb.type === 'checkbox' && cb.classList.contains('tree-checkbox') || cb.classList.contains('dir-checkbox-inner')) {
                if (cb.checked) {
        ...
        this._boundClick = function (e) {
            ...
            if (btn) {
                ...
                var children = li && li.querySelector('.tree-children');
                if (children) {
        ...
        try {
            ...
            if (mode === 'tree') {
                ...
                if (endpoints.selected) {
                    var resps = await Promise.all([fetch(endpoints.list), fetch(endpoints.selected)]);
                    treeResp = resps[0]; selResp = resps[1];
                } else {
                ...
                var tree = data.tree || (data.data && data.data.dirs ? data.data.dirs.map(function (d) { return { name: d, path: d }; }) : []);
                if (tree.length === 0) {
                ...
                if (selResp) {
                    ...
                    var selected = selData.data && selData.data.selected ? selData.data.selected : [];
                    selected.forEach(function (d) { this.selected.add(d); }.bind(this));
                } else {
    ...
    _renderTreeNodes(nodes) {
        ...
        for (var i = 0; i < nodes.length; i++) {
            ...
            var isChecked = self.selected.has(node.path);
            var childrenHtml = hasChildren
                ? '<ul class="tree-children" style="display:none;">' + self._renderTreeNodes(node.children) + '</ul>'
                : '';
            html += '<li class="tree-node">' +
                '<div class="tree-item">' +
                    ...
                    '<input type="checkbox" class="tree-checkbox" value="' + escAttr(node.path) + '" ' +
                        (isChecked ? 'checked' : '') + '>' +
    ...
    _renderFlat(dirs) {
        ...
        for (var i = 0; i < dirs.length; i++) {
            var d = dirs[i];
            var isChecked = self.selected.has(d);
            html += '<label class="dir-checkbox">' +
                '<input type="checkbox" class="dir-checkbox-inner" value="' + escAttr(d) + '" ' +
                    (isChecked ? 'checked' : '') + '>' +
                escHtml(d) +
            '</label>';
        }
