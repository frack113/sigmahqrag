...
def fetch_github_repositories(grid_param: Any = None) -> List[Dict[str, Any]]:
    """Fetch GitHub repositories using the data service."""
    from ..models.data_service import DataService

    # Use provided grid parameter or fall back to global grid
    grid_to_use = grid_param if grid_param is not None else grid

    try:
        if (
            grid_to_use is None
            or not hasattr(grid_to_use, "options")
            or not grid_to_use.options.get("rowData")
        ):
            return []

        if grid_to_use is None or not hasattr(grid_to_use, "options"):
            return []

        repo_config = {"repositories": grid_to_use.options["rowData"]}
...
def is_duplicate(
    url: str, branch: str, exclude_index: int = -1, grid_param: Any = None
) -> bool:
    ...
    for i, repo in enumerate(grid_to_use.options["rowData"]):
        if i == exclude_index:
            continue
        if (
...
def add_repository(
    url: str, branch: str, extensions: str, enabled: bool, grid_param: Any = None
) -> bool:
    ...
    if is_duplicate(url, branch, grid_param=grid_to_use):
        ui.notify(
            f"Repository '{url}' with branch '{branch}' already exists!",
            type="negative",
        )
        return False
    ...
    if grid_to_use is None or not hasattr(grid_to_use, "options"):
        grid_to_use.options = {"rowData": []}

    new_id = len(grid_to_use.options["rowData"])
    grid_to_use.options["rowData"].append(
        {
            "id": new_id,
            "url": url,
            "branch": branch,
...
def update_repository(
    ...
    if is_duplicate(url, branch, index, grid_param=grid_to_use):
        ui.notify(
            f"Repository '{url}' with branch '{branch}' already exists!",
            type="negative",
        )
    ...
    if (
        not grid_to_use
        or not hasattr(grid_to_use, "options")
        or index >= len(grid_to_use.options["rowData"])
    ):
        return False

    grid_to_use.options["rowData"][index] = {
        "id": index,
        "url": url,
        "branch": branch,
        "extensions": extensions if extensions else "None",
...
def remove_repository(index: int, grid_param: Any = None) -> bool:
    ...
    try:
        ...
        repo = grid_to_use.options["rowData"][index]
        url = repo["url"]
        ...
        grid_to_use.options["rowData"].pop(index)
        # Update IDs after removal
        for i, row in enumerate(grid_to_use.options["rowData"]):
            if "id" in row:
                row["id"] = i
...
def refresh_grid():
    ...
    # Only load config if grid is empty (initialization)
    if len(grid.options["rowData"]) == 0:
        config = load_config()
        grid_rows = []
...
def add_new_repository():
    ...
    grid.options["rowData"].append(
        {"id": new_id, "url": "", "branch": "", "extensions": ext_text, "enabled": True}
    )
...
def initialize_page():
    ...
    with ui.card().classes("p-4 w-full max-w-6xl mx-auto"):
        ...
        with ui.row().classes("mt-4"):
            ui.button(icon="add", text="Add New", on_click=lambda: add_new_repository())
            ui.button(icon="save", text="Save", on_click=lambda: save_edits())
            ui.button(icon="delete", text="Delete Selected", color="red").on_click(
                lambda: delete_selected_rows(grid)
            )
...
def handle_cell_edit(event):
    ...
    try:
        ...
        if (
            grid is not None
            and hasattr(grid, "options")
            and 0 <= row["id"] < len(grid.options["rowData"])
        ):
...
def handle_row_click(event):
    ...
    # TODO: Implement row click handling for edit/remove actions
    # This could open a dialog or trigger specific actions based on the clicked row
    pass
...
async def delete_selected_rows(grid):
    ...
    with dialog, ui.card().classes("p-4"):
        ui.label("Are you sure you want to delete the selected repositories?").classes(
            "text-lg font-semibold"
        )
        with ui.row().classes("justify-end mt-4 gap-2"):
            ui.button("Cancel", on_click=dialog.close).props("outline")
            ui.button(
                "Delete", color="red", on_click=lambda: confirm_delete(grid, dialog)
            )
...
async def confirm_delete(grid, dialog):
    ...
    try:
        ...
        grid.options["rowData"] = [
            ...
            if row["id"] not in [r["id"] for r in selected_rows]
        ]
