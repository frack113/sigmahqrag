...
from src.core import ChatService, LLMService, RAGService
...
class Application(BaseService):
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            healthy_services = sum(
                1 for service in self.services if self._is_service_healthy(service)
            )
            ...
            if healthy_services == len(self.services):
                self.logger.info(
                    f"Application initialized successfully with {len(self.services)} services"
                )
                return True
            else:
                self.logger.warning(
                    f"Application initialized with {len(self.services) - healthy_services} unhealthy services"
                )
                return False
    ...
    async def _initialize_core_services(self) -> None:
        ...
        self.chat_service = ChatService(
            llm_service=self.llm_service, rag_service=self.rag_service, **chat_config
        )
    ...
    async def _initialize_infrastructure_services(self) -> None:
        ...
        file_processor_config = self.config.get(
            "file_processor", DEFAULT_CONFIG["performance"]
        )
        self.file_processor = FileProcessor(**file_processor_config)
        self.services.append(self.file_processor)

        # Initialize file storage
        file_storage_config = self.config.get(
            "file_storage", DEFAULT_CONFIG["performance"]
        )
        ...
        lm_studio_config = self.config.get(
            "lm_studio_client", DEFAULT_CONFIG["performance"]
        )
    ...
    def get_overall_health(self) -> dict[str, Any]:
        ...
        healthy_services = sum(
            ...
            if health.get("status") == STATUS_HEALTHY
        )
        ...
        critical_failures = [
            ...
            and service_health[name].get("status") == STATUS_UNHEALTHY
        ]
        ...
        try:
            ...
            for service in reversed(self.services):
                try:
                    ...
                    self.logger.error(
                        f"Error cleaning up service {service.service_name}: {e}"
                    )
