...
def test_basic_usage():
    ...
    response = llm_service.simple_completion(
        "Explain gravity", "You are a physics teacher explaining concepts to beginners."
    )
    print(
...
async def test_streaming():
    ...
    async for chunk in llm_service.streaming_completion(
        "Write a short poem about programming", "You are a creative writer"
    ):
...
def test_service_variants():
    ...
    response = completion_service.simple_completion(
    ...
    response = creative_service.simple_completion(
        "Write a fantasy story about a dragon programmer"
    )
    print(
        f"   Response: {response[:200]}..."
        if len(response) > 200
